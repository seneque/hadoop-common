...
public class FairSchedulerMetrics extends SchedulerMetrics {
  ...
  private long getMemorySize(Schedulable schedulable, Metric metric) {
    if (schedulable != null) {
      switch (metric) {
      case DEMAND:
        return schedulable.getDemand().getMemory();
      case USAGE:
        return schedulable.getResourceUsage().getMemory();
      case MINSHARE:
        return schedulable.getMinShare().getMemory();
      case MAXSHARE:
        return schedulable.getMaxShare().getMemory();
      case FAIRSHARE:
        return schedulable.getFairShare().getMemory();
      default:
        return 0L;
      }
    }
